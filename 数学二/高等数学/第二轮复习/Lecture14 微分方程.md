...
+ 先根据原来表达式设出sin$\beta$+cos$\beta$的形式(如果只有sin或者cos，也得设出这种形式)
+ sin和cos前面多项式的系数就是原来前面最高的多项式的形式。
+ 然后再乘以$e^{ax}$
+ 最后再看是乘x的几次方。看$\alpha+i\beta是不是特征方程的根$。（这里只有0和1的说法，**还得是虚根，实根一定不能**）
...
一个非齐次解+一个齐次解一定是非齐次解。(本质上就是令通解中的一个C为0，一个C的系数是1)

### 已知特解求方程（常系数）

+ 想办法找出齐次的两个特解（分别乘以系数就行了，当然这里用不着）。
+ 用非齐次的特解相减，相减之后的形式最好是一项式。（如果不是一项式该如何处理呢？？）

+ 从而能够定出齐次方程。在把非齐的特解带入非齐的方程。
...
+ 已知三个线性无关的非齐次特解，可以两两相减。如果这两个两两相减的值不是线性相关的，则可以根据其设求出齐次方程的通解。然后再加上另外一个特解就是非齐次方程的通解(这里就不需要设出特解的形式了)。
+ 如果已知的是方程，就需要根据方程求出齐次通解。另外设一个非齐次特解。将特解带进方程求出系数。f